...
Usage: Browse  -i=INPUT_PATH_TO_SIMULATED_NUCLEI_DIR -n=RUN_NUMBER -e=EVENT_NUMBER
...
def main():
	try:
	    ...
	    run_nmb = int(arguments['--run_number'])
	    event_nmb = int(arguments['--event_number'])
	    event_index = event_nmb-1 

	    run = plp.IdealizedPlenoscope.Run(os.path.join(path_to_sim_nuclei,'run%i/'%run_nmb))
